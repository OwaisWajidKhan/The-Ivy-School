...
- [x] **Production login fix (PR #3, `e0192c8`)**: root cause = `TURSO_DATABASE_URL` empty in Vercel env → local `node:sqlite` backend can't write on Vercel's read-only FS → generic 500 on every login. Hardened: startup logs a clear `[CONFIG]` warning when `VERCEL=1` lacks `TURSO_DATABASE_URL`; `/api/health` now reports the active DB backend (`turso` vs `local`), is exempt from the readiness gate, and never 500s (returns 503 + `configError` hint on misconfig). Verified locally: VERCEL=1 no Turso → `db:"local"` + `configError`; with Turso URL → `db:"turso"`.
- [ ] **Deploy config (mostly done — Vercel env vars pending)**: Turso DB `ivy-school` provisioned + seeded on the cloud (`libsql://ivy-school-owaiskhan17.aws-ap-northeast-1.turso.io`; 40 students, 8 users, 2900 attendance rows; one-time local seed took ~12 min over HTTPS). Root `.env` now has the real `TURSO_DATABASE_URL` + `TURSO_AUTH_TOKEN`; `TursoToken.md` (secret) is gitignored. **The Turso web console is now https://app.turso.tech (NOT console.turso.io — retired).** Remaining: set `TURSO_DATABASE_URL`/`TURSO_AUTH_TOKEN`/`JWT_SECRET`/`JWT_REFRESH_SECRET`/`BLOB_READ_WRITE_TOKEN`/`FRONTEND_URL` as Vercel env vars, redeploy, confirm `GET /api/health` shows `db:"turso"` with no `configError`.

### Client Feedback Fixes (COMPLETE - done on `feat/feedback-fixes`)
> Addresses all 9 items in `docs/FEEDBACK.md`. Frontend `vite build` green; backend verified live.
- [x] **Timezone (items 2, 8, 9):** new `backend/src/utils/timezone.js` - single source of truth (settings `school_timezone`, env `APP_TIMEZONE`, server tz, UTC). `helpers.nowStr/todayStr` delegate to it; `toDbString` normalizes device timestamps on `/attendance/scan`; `toLocalSql` converts UTC `datetime('now')` values on read (admin audit/last_login, notifications). Seed writes the real tz (not `UTC`); frontend date-input defaults changed from UTC `toISOString().slice(0,10)` to `todayStr()`.
- [x] **User avatar (items 1, 7):** `users.avatar` column (migration); `PUT /api/auth/me` (email/password) + `PUT /api/auth/me/avatar` (multipart); admin `POST /admin/users` accepts optional `avatar` file; new `frontend/src/pages/Profile.jsx` + `/profile` route; header + profile dropdown show the avatar image when set.
- [x] **Assign-card dropdown (item 4):** `/cards/pool` returns ALL active people with `has_active_card` (was only no-card people, which is always empty because seed gives everyone a card); `/assign` deactivates the person's prior active card and updates the correct student/employee rows.
- [x] **Attendance export (item 3):** `GET /api/attendance/export` + `GET /api/attendance/export-logs` (CSV); buttons on Attendance Summary + Raw Logs; Reports export switched from `window.open` (dropped the Bearer header -> 401) to authenticated blob downloads.
- [x] **Bulk import cards (item 5):** downloadable template, `.csv` file upload, format/validation rules, lowercase `card_type` whitelist, per-row error report (`/bulk`).
- [x] **Scan History page (item 6):** new `frontend/src/pages/ScanLogs.jsx` + `/scans` route + "Scan History" sidebar item; shows id, card UID, person, type, IN/OUT, device, location, time with export; Raw-Logs tab gained Card ID + Type columns.
