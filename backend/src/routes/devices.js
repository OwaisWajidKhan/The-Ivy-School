...
router.get('/', requirePermission('manage_devices'), (req, res) => {
  ok(res, db.prepare('SELECT * FROM devices ORDER BY id DESC').all());
});

// Register / update device
router.post('/', requirePermission('manage_devices'), (req, res) => {
  ...
  if (existing) {
    db.prepare("UPDATE devices SET device_name=?, location=?, status='online', last_sync_time=datetime('now') WHERE id=?")
      .run(device_name || existing.device_name, location || existing.location, existing.id);
    return ok(res, db.prepare('SELECT * FROM devices WHERE id = ?').get(existing.id));
  }
  ...
  ok(res, db.prepare('SELECT * FROM devices WHERE id = ?').get(info.lastInsertRowid), 201);
});

// Update device
router.put('/:id', requirePermission('manage_devices'), (req, res) => {
  const existing = db.prepare('SELECT * FROM devices WHERE id = ?').get(req.params.id);
  if (!existing) return fail(res, 'Device not found', 404);
  const b = req.body;
  db.prepare('UPDATE devices SET device_name=?, location=?, status=? WHERE id=?')
  ...
  ok(res, db.prepare('SELECT * FROM devices WHERE id = ?').get(existing.id));
});

// Delete device
router.delete('/:id', requirePermission('manage_devices'), (req, res) => {
