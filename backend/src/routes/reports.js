...
function buildRows({ personType, from, to }) {
  return db.prepare(
    `SELECT a.*,
...
// Daily report
router.get('/daily', requirePermission('view_reports'), (req, res) => {
...
router.get('/monthly', requirePermission('view_reports'), (req, res) => {
  ...
  const holidays = db.prepare("SELECT COUNT(*) AS c FROM holidays WHERE date BETWEEN ? AND ?").get(from, to).c;
  ok(res, { month, year, from, to, holidays, rows });
});

// Shift report (staff)
router.get('/shift', requirePermission('view_reports'), (req, res) => {
...
// Overtime report
router.get('/overtime', requirePermission('view_reports'), (req, res) => {
  const { from, to } = parseRange(req.query);
  const rows = db.prepare(
...
router.get('/late', requirePermission('view_reports'), (req, res) => {
  const { from, to } = parseRange(req.query);
  const rows = buildRows({ personType: req.query.person_type, from, to }).filter(r => r.late_minutes > 0);
  ok(res, { from, to, rows });
});

// Early exit report
router.get('/early-exit', requirePermission('view_reports'), (req, res) => {
  const { from, to } = parseRange(req.query);
  const rows = buildRows({ personType: req.query.person_type, from, to }).filter(r => r.early_exit_minutes > 0);
  ok(res, { from, to, rows });
});
...
router.get('/export/csv', requirePermission('export_reports'), (req, res) => {
  const { month, year, person_type } = req.query;
  const m = String(parseInt(month) || new Date().getMonth() + 1).padStart(2, '0');
  const y = parseInt(year) || new Date().getFullYear();
  const from = `${y}-${m}-01`;
  const to = `${y}-${m}-31`;
  const rows = buildRows({ personType: person_type || null, from, to });
...
router.get('/export/daily-csv', requirePermission('export_reports'), (req, res) => {
...
router.get('/gate-passes', requirePermission('view_reports'), (req, res) => {
...
router.get('/leaves', requirePermission('view_reports'), (req, res) => {
  const { from, to } = parseRange(req.query);
  const rows = db.prepare(
...
router.get('/export/generic', requirePermission('export_reports'), (req, res) => {
