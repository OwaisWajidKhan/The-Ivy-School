...
router.get('/', (req, res) => {
  ...
  const total = db.prepare(`SELECT COUNT(*) AS c FROM gate_passes gp JOIN students s ON s.id = gp.student_id ${whereSql}`).get(...params).c;
  const rows = db.prepare(`${selectBase} ${whereSql} ORDER BY gp.exit_date DESC, gp.id DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
...
router.post('/', (req, res) => {
  ...
  notifyAdminsGatePass(student, b.reason, b.exit_date);
  ok(res, db.prepare(`${selectBase} WHERE gp.id = ?`).get(info.lastInsertRowid), 201);
});
...
router.put('/:id/status', requirePermission('approve_leave'), (req, res) => {
  ...
  db.prepare("UPDATE gate_passes SET status = ?, approved_by = ?, approved_at = datetime('now') WHERE id = ?")
    .run(status, req.user.id, pass.id);

  const student = db.prepare('SELECT * FROM students WHERE id = ?').get(pass.student_id);
  ...
  notifyPerson({ personType: 'student', personId: pass.student_id, type: 'gate_pass', title, message });
  if (student.parent_contact) {
    notifyEmail({ to: student.parent_contact, recipientType: 'parent', recipientId: pass.student_id, type: 'gate_pass', title, message });
  }
  audit(req.user, 'review_gate_pass', 'gate_pass', pass.id, { status, student: student.full_name }, req.ip);
  ok(res, db.prepare(`${selectBase} WHERE gp.id = ?`).get(pass.id));
});
...
router.post('/verify-exit', (req, res) => {
  ...
  const card = db.prepare('SELECT * FROM rfid_cards WHERE uid = ? AND card_type = ?').get(String(uid), 'student');
  if (!card) return fail(res, 'Unknown student RFID card', 404);
  const student = db.prepare('SELECT * FROM students WHERE id = ?').get(card.person_id);
  if (!student) return fail(res, 'Student not found', 404);
  ...
  db.prepare("UPDATE gate_passes SET status = 'used', used_at = datetime('now'), verified_by = ? WHERE id = ?")
    .run(req.user.id, pass.id);

  // Record an attendance OUT for the student today (early exit) if a summary exists
  const summary = db.prepare("SELECT * FROM attendance_summary WHERE person_type='student' AND person_id=? AND date=?").get(student.id, todayStr());
  if (summary && !summary.out_time) {
    db.prepare("UPDATE attendance_summary SET out_time = strftime('%H:%M', 'now'), status='early_exit' WHERE id=?").run(summary.id);
  }
  ...
  notifyPerson({ personType: 'student', personId: student.id, type: 'gate_pass', title: 'Gate pass used', message });
  audit(req.user, 'use_gate_pass', 'gate_pass', pass.id, { student: student.full_name, uid }, req.ip);
  ok(res, { ...db.prepare(`${selectBase} WHERE gp.id = ?`).get(pass.id), verifiedAt: nowT });
});

// Printable QR slip (HTML, printer friendly)
router.get('/:id/slip', (req, res) => {
  const pass = db.prepare(`${selectBase} WHERE gp.id = ?`).get(req.params.id);
  if (!pass) return fail(res, 'Gate pass not found', 404);
  res.send(renderSlip(pass));
});
...
function notifyAdminsGatePass(student, reason, exitDate) {
  ...
  for (const a of admins) {
    notifyInApp({ recipientType: 'admin', recipientId: a.id, type: 'gate_pass', title, message });
    if (a.email) notifyEmail({ to: a.email, recipientType: 'admin', recipientId: a.id, type: 'gate_pass', title, message });
  }
}

function renderSlip(pass) {
  const school = getSetting('school_name', 'The Ivy School');
