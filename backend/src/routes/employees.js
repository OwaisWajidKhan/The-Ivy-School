...
router.get('/', requirePermission('manage_employees'), async (req, res) => {
  ...
  const total = (await db.prepare(`SELECT COUNT(*) AS c FROM employees e ${whereSql}`).get(...params)).c;
  const rows = await db.prepare(`${selectBase} ${whereSql} ORDER BY e.full_name LIMIT ? OFFSET ?`).all(...params, limit, offset);
...
router.post('/', upload.single('photo'), requirePermission('manage_employees'), async (req, res) => {
  const b = req.body;
...
router.put('/:id', upload.single('photo'), requirePermission('manage_employees'), async (req, res) => {
...
router.delete('/:id', requirePermission('manage_employees'), async (req, res) => {
  const existing = await db.prepare('SELECT * FROM employees WHERE id = ?').get(req.params.id);
  if (!existing) return fail(res, 'Employee not found', 404);
  await db.prepare('DELETE FROM employees WHERE id = ?').run(existing.id);
