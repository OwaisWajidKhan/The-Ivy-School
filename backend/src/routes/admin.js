...
router.put('/users/:id', requireRole('super_admin', 'school_admin'), async (req, res) => {
  ...
  const b = req.body;
  if (b.password) {
    await db.prepare('UPDATE users SET password_hash = ? WHERE id = ?').run(bcrypt.hashSync(b.password, 10), user.id);
  }
  if (b.role) {
    const roleRow = await db.prepare('SELECT id FROM roles WHERE name = ?').get(b.role);
    if (roleRow) await db.prepare('UPDATE users SET role_id = ? WHERE id = ?').run(roleRow.id, user.id);
  }
  await db.prepare('UPDATE users SET email = COALESCE(?, email), status = COALESCE(?, status), person_type = COALESCE(?, person_type), person_id = COALESCE(?, person_id) WHERE id = ?')
...
router.get('/branding', async (req, res) => {
  ...
  const obj = {};
  for (const k of keys) obj[k] = await getSetting(k, null);
  ok(res, obj);
});
